...
```
terminal(command="hermes chat -q --source tool 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="hermes chat -q --source tool 'Set up CI/CD for ~/myapp'", background=true)
```
...
- **Use `hermes chat -q --source tool` for agent-created fire-and-forget runs** — no PTY needed, and the subordinate transcript stays out of normal session trees
