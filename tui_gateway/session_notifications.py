...
def _kb_changes_requested(task, payload: dict, title: str) -> str:
    ...
    return f" review requested changes/BLOCK: {reason}"
...
def _kb_block_loop_detected(task, payload: dict, title: str) -> str:
    ...
    return f" routed to TRIAGE — needs a human decision{count}{reason}"
...
def _kb_poll_board(_kb, slug: str, session_key: str) -> list:
    ...
    with contextlib.closing(conn):
        ...
        for sub in subs:
            ...
            kinds = _kb.effective_notify_event_kinds(sub, _KANBAN_NOTIFY_KINDS)
            _old, _new, events = _kbn.claim_unseen_events_for_sub(
                conn, kinds=kinds, **sub_ident,
            )
            if not events:
                continue
