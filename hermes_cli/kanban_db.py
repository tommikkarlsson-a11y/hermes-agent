...
NOTIFY_EVENT_KIND_ORDER = (
    "completed", "blocked", "gave_up", "crashed", "timed_out", "status",
    "archived", "unblocked", "review_requested", "changes_requested",
    "block_loop_detected",
)
VALID_NOTIFY_EVENT_KINDS = frozenset(NOTIFY_EVENT_KIND_ORDER)
_NOTIFY_EVENT_KINDS_KEY = "kanban_event_kinds"
...
def normalize_notify_event_kinds(kinds: Iterable[str]) -> tuple[str, ...]:
    """Validate and canonically order a non-empty notification event filter."""
    if isinstance(kinds, (str, bytes)):
        raise ValueError("notify_event_kinds must be a non-empty list of event kinds")
    try:
        normalized = tuple(str(kind).strip().lower() for kind in kinds)
    except TypeError as exc:
        raise ValueError(
            "notify_event_kinds must be a non-empty list of event kinds"
        ) from exc
    if not normalized:
        raise ValueError("notify_event_kinds must be a non-empty list of event kinds")
    if any(not kind or kind not in VALID_NOTIFY_EVENT_KINDS for kind in normalized):
        raise ValueError(
            "notify_event_kinds must contain only supported Kanban event kinds"
        )
    if len(set(normalized)) != len(normalized):
        raise ValueError("notify_event_kinds must not contain duplicates")
    order = {kind: index for index, kind in enumerate(NOTIFY_EVENT_KIND_ORDER)}
    return tuple(sorted(normalized, key=order.__getitem__))


def effective_notify_event_kinds(sub: Any, default: Iterable[str]) -> tuple[str, ...]:
    """Return a subscription filter, or ``default`` for legacy/malformed rows."""
    ...
    if not isinstance(metadata, dict):
        return fallback
    raw = metadata.get(_NOTIFY_EVENT_KINDS_KEY)
            ...
            with write_txn(conn, allow_nested=True):
                ...
                if task_status == "blocked":
                    _append_event(
                        ...
                        task_id,
                        "blocked",
                        {"reason": "initial_status", "status": "blocked", "actor": created_by or "user", "initial": True},
                    )
                # ACK-edge: the originating channel hears a child BLOCK, not just the fan-in.
                inherit_creator_origin(conn, task_id, creator_task_id, created_at=now)
                _inherit_notify_subs(
                    conn, task_id, parents, created_at=now,
...
def _inherit_notify_subs(
    ...
    if notify_event_kinds is not None:
        encoded_kinds = ",".join(notify_event_kinds)
        rows = conn.execute(
            "SELECT platform, chat_id, thread_id, delivery_metadata "
            "FROM kanban_notify_subs WHERE task_id = ?", (child_id,),
        ).fetchall()
        for sub in rows:
            metadata = _json_dict(sub["delivery_metadata"])
            metadata[_NOTIFY_EVENT_KINDS_KEY] = encoded_kinds
            conn.execute(
                "UPDATE kanban_notify_subs SET delivery_metadata = ? "
                "WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?",
                (
                    json.dumps(metadata, sort_keys=True, separators=(",", ":")),
                    child_id, sub["platform"], sub["chat_id"], sub["thread_id"],
                ),
            )
