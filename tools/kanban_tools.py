...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        new_tid = kb.create_task(
            ...
            notify_event_kinds=notify_event_kinds,
        ...
        return _ok(
            ...
            subscribed=_maybe_auto_subscribe(
                conn, new_tid, notify_event_kinds=notify_event_kinds,
            ),
        )
...
def _maybe_auto_subscribe(
    conn: Any, task_id: str, *, notify_event_kinds: Any = None,
) -> bool:
    ...
    try:
        ...
        if notify_event_kinds is not None:
            normalized = _kb.normalize_notify_event_kinds(notify_event_kinds)
            metadata = dict(target.get("delivery_metadata") or {})
            metadata["kanban_event_kinds"] = ",".join(normalized)
            target["delivery_metadata"] = metadata
