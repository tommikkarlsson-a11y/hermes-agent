...
KANBAN_GUIDANCE = (
    ...
    "5. **Finish only your own task.** Include the structured handoff "
    "(`summary`, `metadata`) on your own lifecycle transition; never put "
    "secrets, tokens, or raw PII in these durable fields. Use "
    "`kanban_complete`, `kanban_block`, `kanban_request_review`, or "
    "`kanban_request_changes` only for the task you were assigned. The "
    "dispatcher and the pre-built task graph handle downstream routing.\n"
    "6. **Do not mutate or coordinate the task graph.** A dispatcher worker "
    "must not create, assign, link, unlink, unblock, inspect, or coordinate "
    "other cards, and must not use `delegate_task`. If follow-up work appears, "
    "record it in your own completion handoff; a direct orchestrator control "
    "session may build or revise the graph before release.\n"
    "7. **Flag collision hotspots on your own card.** If your change keeps "
    "colliding in one file, leave a `kanban_comment` starting with "
    "`hotspot: <path> — <one-line reason>` on your task and repeat the flag "
    "in completion metadata. Do not inspect or modify sibling cards.\n"
    "\n"
    ...
    "- Do not create, assign, link, unlink, unblock, inspect, or coordinate "
    "other cards. Do not call `delegate_task`.\n"
)
