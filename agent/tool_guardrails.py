...
class ToolCallGuardrailController:
    ...
    def _decide(
        ...
        *, message: str | None = None, **fmt: Any,
        ...
        """Build a warn/block/halt decision; only ``halt`` stops the turn."""
        if message is None:
    ...
    def _check_loop_cap(
        ...
        """Halt once a per-turn cap is reached (BEFORE the call, so the (cap+1)-th is refused), else advance
        ...
        if increment and cap and count >= cap:
            return self._decide("halt", code, tool_name, count, signature, cap=cap)
